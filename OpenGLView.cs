...
namespace SuperBitBros
{
    abstract class OpenGLView
    {
        ...
        protected virtual void RenderRectangle(Rectangle2d rect, OGLTexture texture, double distance)
        {
            ...
            GL.Begin(BeginMode.Polygon);
            GL.TexCoord2(coords.bl); GL.Vertex3(rect.tl.X, rect.tl.Y, distance);
            GL.TexCoord2(coords.tl); GL.Vertex3(rect.bl.X, rect.bl.Y, distance);
            GL.TexCoord2(coords.tr); GL.Vertex3(rect.br.X, rect.br.Y, distance);
            GL.TexCoord2(coords.br); GL.Vertex3(rect.tr.X, rect.tr.Y, distance);
            GL.End();
        }
    }
}